I18N.register({
  "en": {
    "brand": "Utilities",
    "nav_dice": "Dice Calculator",
    "nav_color": "Color Picker",
    "nav_json": "JSON Visualizer",
    "nav_dayinfo": "Day Info",
    "nav_repayment": "Repayment",
    "nav_qr": "QR Generator",
    "nav_exchange": "Exchange Rate",
    "nav_slideshow": "Folder Image Slideshow",
    "settings": "Settings",
    "settings_lang": "Language",
    "lang_en": "English",
    "lang_ko": "Korean",
    "lang_jp": "Japanese",
    "settings_defaultpage": "Default Page",
    "page_last": "Last visited page",
    "page_dice": "Dice Calculator",
    "page_color": "Color Picker",
    "page_json": "JSON Visualizer",
    "page_dayinfo": "Day Info",
    "page_repayment": "Repayment",
    "page_qr": "QR Generator",
    "page_exchange": "Exchange Rate",
    "page_slideshow": "Folder Image Slideshow",
    "settings_screenmode": "Screen Mode",
    "mode_auto": "Auto",
    "mode_light": "Light",
    "mode_dark": "Dark"
  },
  "ko": {
    "brand": "Utilities",
    "nav_dice": "주사위 계산기",
    "nav_color": "색상 피커",
    "nav_json": "JSON 시각화",
    "nav_dayinfo": "날짜 정보",
    "nav_repayment": "대출 상환",
    "nav_qr": "QR 생성기",
    "nav_exchange": "환율 계산기",
    "nav_slideshow": "폴더 이미지 슬라이드쇼",
    "settings": "설정",
    "settings_lang": "언어",
    "lang_en": "영어",
    "lang_ko": "한국어",
    "lang_jp": "일본어",
    "settings_defaultpage": "기본 페이지",
    "page_last": "마지막 페이지",
    "page_dice": "주사위 계산기",
    "page_color": "색상 피커",
    "page_json": "JSON 시각화",
    "page_dayinfo": "날짜 정보",
    "page_repayment": "대출 상환",
    "page_qr": "QR 생성기",
    "page_exchange": "환율 계산기",
    "page_slideshow": "폴더 이미지 슬라이드쇼",
    "settings_screenmode": "화면 모드",
    "mode_auto": "자동",
    "mode_light": "라이트",
    "mode_dark": "다크"
  },
  "jp": {
    "brand": "Utilities",
    "nav_dice": "サイコロ計算機",
    "nav_color": "カラーピッカー",
    "nav_json": "JSONビジュアライザー",
    "nav_dayinfo": "日付情報",
    "nav_repayment": "ローン返済",
    "nav_qr": "QRジェネレーター",
    "nav_exchange": "為替レート計算機",
    "nav_slideshow": "フォルダ画像スライドショー",
    "settings": "設定",
    "settings_lang": "言語",
    "lang_en": "英語",
    "lang_ko": "韓国語",
    "lang_jp": "日本語",
    "settings_defaultpage": "デフォルトページ",
    "page_last": "最後のページ",
    "page_dice": "サイコロ計算機",
    "page_color": "カラーピッカー",
    "page_json": "JSONビジュアライザー",
    "page_dayinfo": "日付情報",
    "page_repayment": "ローン返済",
    "page_qr": "QRジェネレーター",
    "page_exchange": "為替レート計算機",
    "page_slideshow": "フォルダ画像スライドショー",
    "settings_screenmode": "画面モード",
    "mode_auto": "自動",
    "mode_light": "ライト",
    "mode_dark": "ダーク"
  }
});
